...
LevelBus.prototype.addChannel = function(name, handler, outputs, opts, cb) {
    ...
    this._flush(name, Date.now(), cb);
}

LevelBus.prototype.getChannel = function(name) {
    if(this.channels[name] === undefined) throw new Error("No such channel: " + name);
    return this.channels[name];
}

LevelBus.prototype.pauseChannel = function(name) {
    this.getChannel(name).pause();
}

/**
 * Flushes jobs that were not processed on a previous run for any reason.
 */
LevelBus.prototype.resumeChannel = function(name, ts, cb) {
    
    var q = this.getChannel(name);

    if(!q.paused) return cb && cb();

    if(typeof(ts) == 'undefined') {
        q.resume();
        return this._flush(name, Date.now(), cb);
    }
}

LevelBus.prototype._flush = function(name, ts, cb) {
    var self = this;
    var q = this.getChannel(name);
        ...
        async.eachSeries(
            ...
            (err) => {
                ...
                if(pending.length == batchSize) {
                    q.drain = this._flush.bind(this, name, ts, cb);
                } else {
                    q.drain = noop;
                    cb();
                }
